...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($creatorName) || empty($email) || empty($password) || empty($contact) || empty($qualifications)) {
        ...
        $sql = "INSERT INTO course_creators (creator_name, email, password, contact, qualifications) VALUES (?, ?, ?, ?, ?)";
        
        $stmt = mysqli_prepare($conn, $sql);
        
        if ($stmt) {
            // Bind parameters and execute the query
            mysqli_stmt_bind_param($stmt, "sssss", $creatorName, $email, $password, $contact, $qualifications);
            if (mysqli_stmt_execute($stmt)) {
                echo "Course creator added successfully.";
            } else {
