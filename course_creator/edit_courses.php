...
function updateCourse($conn, $courseID, $courseName, $courseDescription, $courseObjectives, $publishStatus, $coursePicture, $instructorID) {
    $updateQuery = "UPDATE courses
                    SET CourseName = ?, CourseDescription = ?, CourseObjectives = ?, PublishStatus = ?, CoursePicture = ?, InstructorID = ?
                    WHERE CourseID = ?";
    $stmt = mysqli_prepare($conn, $updateQuery);
    mysqli_stmt_bind_param($stmt, "sssssii", $courseName, $courseDescription, $courseObjectives, $publishStatus, $coursePicture, $instructorID, $courseID);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $coursePicture = $existingCoursePicture; // Initialize with the existing picture name
    ...
    if (updateCourse($conn, $courseID, $courseName, $courseDescription, $courseObjectives, $publishStatus, $coursePicture, $instructorID)) {
...
if (isset($_GET["courseID"]) && !empty($_GET["courseID"])) {
    ...
    $query = "SELECT courses.*, instructors.instructor_id 
              FROM courses 
              LEFT JOIN instructors ON courses.InstructorID = instructors.instructor_id
              WHERE CourseID = ?";
...
<div class="container mt-5 mb-4">
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <?php if (!empty($existingCoursePicture)) : ?>
                        <img src="../admin/uploads/<?php echo $existingCoursePicture; ?>" alt="Course Picture" width="150">
                    <?php else : ?>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="instructorID">
                        ...
                        while ($instructor = mysqli_fetch_assoc($instructorResult)) {
                            $selected = ($instructor["instructor_id"] == $instructorID) ? "selected" : "";
        ...
        <div class="form-group">
            <input type="submit" class="float-right mb-5 btn btn-primary" value="Update Course">
        </div>
    </form>
</div>
